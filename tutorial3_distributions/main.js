...
/* CONSTANTS AND GLOBALS */
const width1 = window.innerWidth * 1,
  height1 = window.innerHeight * 0.8,
  margin1 = { top: 20, bottom: 60, left: 60, right: 40 },
  radius1 = 5;

/* APPLICATION STATE */
let state = {
  data: [],
  selectedState: "All" // + YOUR INITIAL FILTER SELECTION
};

/* LOAD DATA */
d3.csv("../data/2020ElectionMinWage.csv", d3.autoType).then(raw_data => {
  // + SET YOUR DATA PATH
  console.log("data1", raw_data);
  // save our data to application state
  state.data = raw_data;
  init();
});
...
function init() {
  ...
  // + DEFINE SCALES
xScale = d3.scaleLinear()
.domain(d3.extent(state.data, d=>d.PercentofStatePopVoted))
.range([margin.left, width - margin.right])
